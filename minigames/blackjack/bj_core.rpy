...
# test for chatgpt blackjack 7.5.2 fix 2026-06-17
init python:

    def create_deck():
        """Creates and returns a standard 52-card deck."""
        # Ren'Py 7 uses Python 2 here, so the deck uses simple suit names.
        # These names already match the png filenames.
    ...
    def card_to_filename(card):
        """Converts a card tuple to its image filename."""
        rank = card[0]
        suit = card[1]
...
label blackjack_deal_animation:
    # Empty the same hand lists in place so old saves and the screen stay pointed at them.
    $ del player_hand[:]
    $ del dealer_hand[:]
...
label blackjack_end:
    ...
    if result == "player_blackjack":
        ...
        $ result_text = "Blackjack! You win {color=#00FF00}$" + str(blackjack_payout) + "{/color}!"
    elif result == "win":
        play sound sound_bj_win
        $ bj_player_money += total_bet
        $ result_text = "You beat the dealer! You win {color=#00FF00}$" + str(total_bet) + "{/color}!"
    elif result == "lose":
        play sound sound_bj_lose
        $ bj_player_money -= total_bet
        $ result_text = "You lose. The dealer takes {color=#FF0000}$" + str(total_bet) + "{/color}."
    else: # Tie
