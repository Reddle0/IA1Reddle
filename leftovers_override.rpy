...
# 07/07/25 Moved the menu-related (preferences etc.) stuff into a new leftovers_menu.rpy file
...
# ---- styles (put once, e.g., near other style defs) ----
style splash_card is frame:
    background Solid("#0009")
    xpadding 48
    ypadding 36

style splash_title is text:
    size 64
    color "#FFFFFF"
    outlines [ (4, "#000000", 0, 0) ]
    xalign 0.5

style splash_subtitle is text:
    size 36
    color "#EDEDED"
...
style splash_note:
    ...
    color "#FFFFFF"
    outlines [(2, "#000", 0, 0)]
    #line_spacing 2   # optional to match vertical rhythm

screen bullet_list(items, content_w=900, size=24, style_name="splash_note", bullet_w=28, gap=12, bullet_char="•", left_pad=80):

    $ row_w = left_pad + bullet_w + gap + content_w
    ...
    vbox:
        ...
        for s in items:
            fixed:
                ...
                # bullet column (shifted right by left_pad)
                text bullet_char:
                    style style_name
                    size size
                    xpos left_pad
                ...
                # text column (keeps same wrap width, shifted equally)
                text s:
                    style style_name
                    size size
                    xpos left_pad + bullet_w + gap
                    xanchor 0.0
                    xmaximum content_w
                    yalign 0.0
...
# And Supersexual Awakening #
    if os.path.isdir(os.path.join(renpy.config.gamedir, "mods", "supersexual_awakening")):
        $ store.supersexual_awakening_detected = True

    show bg warning

    if os.path.isdir(os.path.join(renpy.config.gamedir, "mods", "pregnancy_epilogue_remake_mod")):
        if store.pregnancy_epilogue_remake_mod_detected and store.supersexual_awakening_detected:
            show screen splash_text_leftovers_cru_supersexual
        elif store.pregnancy_epilogue_remake_mod_detected:
            show screen splash_text_leftovers_cru
    if os.path.isdir(os.path.join(renpy.config.gamedir, "mods", "supersexual_awakening")):
        if store.supersexual_awakening_detected:
            show screen splash_text_leftovers_supersexual
    else:
        show screen splash_text_leftovers

    with dissolve
    $ renpy.pause(2)
    show expression At("splash_ctc", Transform(yoffset=30)) # changed to move the CTC downwards
    $ renpy.pause()
...
init 2:
    screen splash_text_leftovers_supersexual:
        vbox:
            xalign 0.5
            yalign 0.5

            text "{b}Leftovers Mod + Supersexual Awakening detected!{/b}" size 40 xalign 0.5
            text "See below:" size 32 xalign 0.5
            text "Both mods are now active!" size 24 xalign 0.5
            text "- The main menu has also been modified to include \"Mod Select\"." size 24 xalign 0.5
            text "- To see the content featured in Supersexual Awakening, select \"The summer ended! (Start Supersexual Awakening story)\"." size 24 xalign 0.5
            text "- Report any issues to the IA Discord, Vintage Studio's Discord, Reddle's IA1 Mods Empornium or the IA Modding Thread on AllTheFallen." size 24 xalign 0.5
            text "Thank you for playing these mods!" size 24 xalign 0.5

init 2:
    screen splash_text_leftovers_cru_supersexual:
        #frame style "splash_card":
        ##    xalign 0.5
        #    yalign 0.5
        ...
        vbox:
            ...
            #text "Leftovers Mod + Cru's Remake Mod + Supersexual Awakening detected!" #style "splash_title"
            #text "See below:" #style "splash_subtitle"
            #text "All mods are now active!" #style "splash_note" xalign 0.5
            
            text "{b}Leftovers Mod + Cru's Remake Mod + Supersexual Awakening detected!{/b}"  size 40 xalign 0.5
            text "See below:" size 32 xalign 0.5
            text "All mods are now active!" size 24 xalign 0.5

            null height 10
            #text "• To see the content featured in the Pregnancy Epilogue Remake Mod, select \"View the Epilogue\" in Nate's bedroom, and play through it." size 24 xalign 0.5
            #text "• For Supersexual Awakening, select \"The summer ended! (Start Supersexual Awakening story)\"." size 24 xalign 0.5
            #text "• The main menu has also been modified to include \"Mod Select\"." size 24 xalign 0.5
            use bullet_list(items=[
                "To see the content featured in the Pregnancy Epilogue Remake Mod,\nselect \"View the Epilogue\" in Nate's bedroom, and play through it.",
                "For Supersexual Awakening, select \"The summer ended! (Start Supersexual Awakening story)\".",
                "The main menu has also been modified to include \"Mod Select\"."
            ], content_w=900, size=24, left_pad=100)
